...
- 指针越过激活距离后，来源 DOM 隐藏视觉但保留布局占位；同尺寸预览插入 `document.body`，使用 `translate3d()` 跟随指针并显示完整不透明度与阴影。
- Pointer 命中通过 `document.elementsFromPoint()` 找到 droppable；Scope 与 `accepts()` 共同决定能否放下。
...
1. `draggable` 取得自己的 `payload`，通过 Pointer Events 发起页面内拖动并创建 transform 预览。
...
- `draggable` 只拥有 Pointer 拖动会话、transform 预览、拖动状态和 `payload`，不拥有接收端。
...
- 当前文档不设计拖拽排序、目标占位符或 Widget 重排策略；跟随指针的拖动预览属于 draggable 的现役基础视觉。
- 当前文档不建立 `DragAndDrop` Manager、`source()`、`target()` 或业务专用 Scope Plugin。

## 本地 Demo

- [Draggable Demo](../../src/components/plugins/kits/draggable/draggable.demo.tsx)：独立查看 transform 预览、完整不透明度与阴影。
...
- `payload` 是拖动携带物的公开名称。
- 页面内部 draggable 始终使用 Pointer Events 与 transform 预览，不启用浏览器原生 draggable。
