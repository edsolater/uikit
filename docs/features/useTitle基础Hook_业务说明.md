...
- useTitle 基础 hook 的修改计划在 [useTitle基础Hook_修改计划.md](../plans/useTitle基础Hook_修改计划.md) 里单独维护。
- 每次修改 useTitle 的对外语义、边界或浏览器协作方式后，都必须同步更新本文档。

# useTitle 基础 Hook 业务说明
...
- `useTitle` 的主体身份是“浏览器标题同步 hook”。
- `useTitle` 负责两件事：接收调用方传入的新标题，并返回当前浏览器标题。
- `useTitle` 不负责决定标题文案应该来自哪个业务模块。
- `useTitle` 不负责回滚旧标题。
- `useTitle` 不负责 `meta` 标签、分享卡片、路由标题策略或多层标题模板。
...
- 后续如果继续扩展文档标题相关能力，应先判断是不是仍属于 `document.title` 同步边界；超出这个边界的能力不应硬塞回 `useTitle`。