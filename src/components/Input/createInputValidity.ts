...
import { createState, toReadableState, val, type Source, type State } from '../../hooks'
...
export function createValiditor(options: ValidityOptions): Validity {
  const allRulesPassed = toReadableState(options.validIf).map((validIf) => toArray(validIf).every((v) => isValidationPassed(v)))
