...
import { toReadableState, val, type Source } from '../../hooks'
import { Piv } from '../Piv'
...
export function Table<Row extends TableDataRow>(props: TableProps<Row>) {
  ...
  const columnKeys = toReadableState(props.data).map((data) => (props.keys ? val(props.keys) : pickTableColumnKeys(data)))
