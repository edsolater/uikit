/**
 * Dreprecated  just ::-webkit-scrollbar is enough
 */
...
export default function ScrollDiv({
  ...
  useEffect(() => {
    ...
    const controller = addEventListener(
      ...
      ({ el }) => {
        ...
        const { scrollTop: contentScrollTop, scrollLeft: contentScrollLeft } = el
        ...
        setCSSVariable(outerContainerRef.current, '--scroll-top', currentScrollTop)
        setCSSVariable(outerContainerRef.current, '--scroll-left', currentScrollLeft)
  ...
  useEffect(() => {
    const eventId = attachPointerMove(yThumbRef.current, {
      move: ({ currentDeltaInPx }) => {
        ...
        setCSSVariable(outerContainerRef.current, '--scroll-top', (prev) => {
  ...
  useEffect(() => {
    const eventId = attachPointerMove(xThumbRef.current, {
      move: ({ currentDeltaInPx }) => {
        ...
        setCSSVariable(outerContainerRef.current, '--scroll-left', (prev) => {
  ...
  useEffect(() => {
    ...
    setCSSVariable(outerContainerRef.current, '--content-avaliable-scroll-height', String(totalScrollHeight))
    ...
    setCSSVariable(outerContainerRef.current, '--content-avaliable-scroll-width', String(totalScrollWidth))
  ...
  return (
    <Div className_='ScrollDiv' domRef={outerContainerRef} className={['relative', className]}>
      <Div
        className_='ScrollDiv--content'
